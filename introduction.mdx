---
title: "License Plate Detection with SSD MobileNet and TensorFlow"
sidebarTitle: "Introduction"
description: "A TensorFlow 1.x computer vision project that detects vehicle license plates across three modes: static images, pre-recorded video, and live webcam."
---
...
**Image mode** scans a directory for `.jpg` files using `os.listdir()` combined with `endswith('.jpg')`, and runs inference on each file sequentially, displaying results in an OpenCV window. The directory is set via the `--ruta` flag; if omitted, the current working directory is used.
...
1. **Load the label map** — `label_map_util.load_labelmap` reads `labelmap.pbtxt`, `label_map_util.convert_label_map_to_categories` converts it to a list of category dicts, and `label_map_util.create_category_index` builds the integer-to-name mapping used by the visualiser.
